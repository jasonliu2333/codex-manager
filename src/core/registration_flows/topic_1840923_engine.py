"""
topic_1840923 注册流程引擎（session 优先 + OAuth 回退）
"""
...
class Topic1840923RegistrationEngine:
    """
    注册引擎
    ...
    def _start_oauth(self) -> bool:
        ...
        try:
            self._log("开始 OAuth 授权流程 (topic_1840923)...")
    ...
    def _perform_post_registration_oauth(self) -> Optional[Dict[str, Any]]:
        ...
        self._log("回退 OAuth 登录以补全 Token...")
    ...
    def _find_jwt_in_data(data: Any, depth: int = 0) -> Optional[str]:
        ...
        if isinstance(data, dict):
            for value in data.values():
                result = Topic1840923RegistrationEngine._find_jwt_in_data(value, depth + 1)
                if result:
                    return result
        if isinstance(data, list):
            for item in data:
                result = Topic1840923RegistrationEngine._find_jwt_in_data(item, depth + 1)
                if result:
                    return result
        return None
