...
class DefaultRegistrationEngine:
    """
    注册引擎
    ...
    def _start_oauth(self) -> bool:
        """开始 OAuth 流程（默认 auth.openai.com 直连）"""
        try:
    ...
    def retry_last_oauth_callback_token_exchange(self, proxy_url: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """只重试最后一步 callback -> token exchange。"""
    ...
    @staticmethod
    def _find_jwt_in_data(data: Any, depth: int = 0) -> Optional[str]:
        """递归搜索 JSON 内可能的 JWT token。"""
        if depth > 6:
            return None
        if isinstance(data, str):
            ...
            if len(parts) == 3 and len(data) > 100:
                try:
                    ...
                    decoded = _b64.urlsafe_b64decode((payload + padding).encode("ascii"))
        ...
        if isinstance(data, dict):
            for value in data.values():
                result = DefaultRegistrationEngine._find_jwt_in_data(value, depth + 1)
                if result:
                    return result
        if isinstance(data, list):
            for item in data:
                result = DefaultRegistrationEngine._find_jwt_in_data(item, depth + 1)
                if result:
                    return result
        return None

    def _get_chatgpt_session_tokens(self) -> Optional[Dict[str, Any]]:
        """优先复用注册会话，从 chatgpt.com session 提取 token。"""
        try:
            if self._post_signup_continue_url:
                try:
                    response = self.session.get(
                        self._post_signup_continue_url,
                        headers={
                            "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
                            "user-agent": self.http_client.default_headers.get("User-Agent", ""),
                    ...
                    self._log(f"Callback 访问状态: {response.status_code}")
                except Exception as exc:
                    self._log(f"Callback 访问异常: {exc}", "warning")

            referer = self._post_signup_continue_url or "https://chatgpt.com/"
            response = self.session.get(
            ...
            self._log(f"Session 获取状态: {response.status_code}")
            if response.status_code != 200:
                return None
            ...
            session_token = self._get_cookie_value(
            ...
            self._log(f"Session 提取失败: {e}", "warning")
            return None
    ...
    def run(self) -> RegistrationResult:
        ...
        try:
            ...
            self._log("13. 优先复用 Session 获取 Token...")
            session_info = self._get_chatgpt_session_tokens()
            token_info = None

            if session_info:
                token_info = session_info
                raw = session_info.get("raw") or {}
                ...
                result.access_token = session_info.get("access_token", "") or ""
                result.session_token = session_info.get("session_token", "") or ""
                self._log("Session 复用成功，已获取 AccessToken")
            else:
                self._log("Session 复用失败，回退 OAuth 登录...", "warning")
